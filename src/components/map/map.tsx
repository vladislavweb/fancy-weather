  ...
  return (
    <div className="Map-wrapper">
      ...
      <div className='coordinates'>
        <p className='long'>{`${(mapDescription as any)[currentLang].long}: ${longCoord.gradus}° ${longCoord.minutes}' ${longCoord.gradus > 0 ? 'E' : 'W'}`}</p>
        <p className='lati'>{`${(mapDescription as any)[currentLang].lati}: ${latCoord.gradus}° ${latCoord.minutes}' ${latCoord.gradus > 0 ? 'N' : 'S'}`}</p>
      </div>
    </div>
  );
};
